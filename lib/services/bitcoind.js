...
Bitcoin.prototype.getAPIMethods = function() {
  var methods = [
    ...
    ['tokenlist', this, this.tokenlist, 0],
    ['tokeninfo', this, this.tokeninfo, 2],
  ];
  return methods;
};
...
/**
 * Called by Node to get a list of CC tokens.
 * @param {Function} callback
 */
Bitcoin.prototype.tokenlist = function(callback) {
  var self = this;
  this.client.tokenlist(function(err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
    }
    var result = response.result;
...
/**
 * Called by Node to get CC token info by a txid.
 * @param {String} txid - The hex string of the transaction
 * @param {Function} callback
 */
Bitcoin.prototype.tokeninfo = function(txid, callback) {
  var self = this;
  this.client.tokeninfo(txid, function(err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
    }
    var result = response.result;
